...
import { textShadowValuesToCSSArray } from "../../../../lexical/styles/typography";
...
const defaultValue: types.CSSTextShadowValue = {
  offsetX: "0px",
  offsetY: "0px",
  blurRadius: "0px",
  color: "",
...
const Context = createContext<{
  values: types.CSSTextShadowValue[];
  setValues: (values: types.CSSTextShadowValue[]) => void;
}>({} as any);
...
const WrapDraggableBox = (props: {
  ...
  const [value, setValue] = useState<types.CSSTextShadowValue>(defaultValue);

  const updateValue = (newValue: types.CSSTextShadowValue | undefined) => {
    const values = formDataRef.current.__textShadow as
      | types.CSSTextShadowValue[]
      | undefined;
    ...
    updateFormData({
      __textShadow: newValues,
      textShadow: textShadowValuesToCSSArray(newValues),
    });
  ...
  useEffect(() => {
    const newValues = formDataRef.current
      .__textShadow as types.CSSTextShadowValue[];
  ...
  useEffect(() => {
    ...
    savePrevValue((css) => {
      const prevValue = css.__textShadow as
        | types.CSSTextShadowValue[]
        | undefined;
  ...
  const handleChange = (v: Partial<types.CSSTextShadowValue>) => {
    trackEventEnd(
      "text-shadow-change",
      () => {
        const newValue = {
  ...
  const handleCancel = () => {
    ...
    const prevValue = getPrevValue("textShadow");
    if (prevValue && Array.isArray(prevValue) && prevValue.length > 0) {
      updateFormData({
        __textShadow: prevValue,
        textShadow: textShadowValuesToCSSArray(prevValue),
      });
...
export const TextShadow = () => {
  ...
  const [values, setValues] = useState<types.CSSTextShadowValue[]>([]);
  ...
  const handleDelete = (index: number) => {
    ...
    logger.log("Deleting text shadow at index", index, newValues);
    updateFormData({
      __textShadow: newValues,
      textShadow: textShadowValuesToCSSArray(newValues),
    });
  ...
  const handleChangeOrder = (newValues: types.CSSTextShadowValue[]) => {
    updateFormData({
      __textShadow: newValues,
      textShadow: textShadowValuesToCSSArray(newValues),
    });
  ...
  useEffect(() => {
    ...
    const newValues = editor.read(() => selectedNode.getLatest()).__css.get()
      ?.__textShadow as types.CSSTextShadowValue[] | undefined;
  ...
  return (
    <Context value={{ values, setValues }}>
      ...
      >
        ...
        >
          <Typography fontSize={10}>Add Text Shadow</Typography>
        </Button>
        {values.length > 0 && (
          <SortableList
